...
import MusicPlayerScreen from "./MusicPlayerPage/MusicPlayerScreen";
import {
  View,
  Text,
  StyleSheet,
...
const description = `Taylor Swift is an iconic American singer-songwriter known for her narrative songwriting and genre-spanning musical style. Born on December 13, 1989, in Reading, Pennsylvania, she began her career in country music before transitioning to pop, rock, and indie genres. Swift's early work, including her self-titled debut album, showcased her talent for storytelling, often drawing from her personal experiences, relationships, and emotions.
`;

const ArtistDetailPage = ({ artistData }) => {
  const { name, image, followerCount } = artistData;
    ...
    <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
      <View>
          ...
          <View style={styles.buttonContainer}>
            ...
            {isPlaying ? (
              <Pressable onPress={handlePlaying}>
                <FontAwesome name="pause-circle" size={50} style={styles.btn} />
              </Pressable>
            ) : (
              <Pressable onPress={handlePlaying}>
                <FontAwesome name="play-circle" size={50} style={styles.btn} />
              </Pressable>
            )}
          </View>
        </View>
        <View style={{ margin: 24 }}>
          ...
          {albumResultData &&
            albumResultData.map((album) => (
              <Pressable key={album.id} onPress={() => handleSelectSong(album)}>
                <AlbumItem
                  key={album.id}
                  albumData={album}
                  imageWidth={60}
        ...
        <View style={{ margin: 24 }}>
          ...
          <Image
            source={require("@/assets/images/taylor_swift.png")}
        ...
        {selectedAlbum && (
          <View
            style={
              isMinimized === false
                ? styles.fullScreen && { height: screenHeight - 100 }
                : styles.minimizedScreen
            }
          >
