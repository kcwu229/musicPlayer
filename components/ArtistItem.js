...
const ArtistItem = ({
  ...
  const [hasFollowed, setHasFollow] = useState(
    followedArtists != undefined && followedArtists.includes(_id)
  );
  ...
  useEffect(() => {
    if (
      ...
      followedArtists.includes(_id) !== hasFollowed
    ) {
        ...
        {allowFollowButton && userId ? (
          <Pressable onPress={() => handleFollow(artistData._id)}>
            <View
              style={[
                styles.followBtn,
                { borderColor: hasFollowed ? "rgba(255, 0, 0, 0.8)" : "grey" },
              ]}
            >
              <Text
                style={[
